...
const logos = ["PAULWIN S R", "FULL STACK", "DEVELOPER", "SOFTWARE ENGINEER"];
      ...
      <Route path="/" element={
        <div className="min-h-screen bg-black text-white overflow-x-hidden">
          ...
          <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-5 backdrop-blur-xl bg-black/20 border-b border-white/10">
            ...
            <ul className="hidden md:flex items-center gap-10 text-xs tracking-widest text-white/70 uppercase">
              {['Home', 'About', 'Showcase', 'Contact'].map(item => (
                <li
                  key={item}
                  onClick={() => document.getElementById(item === 'Home' ? 'Home' : item.toLowerCase())?.scrollIntoView({ behavior: "smooth" })}
            ...
            <div className="fixed inset-0 z-40 bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center gap-10 text-white uppercase tracking-[0.3em] text-sm md:hidden">
              ...
              {['Home', 'About', 'Showcase', 'Contact'].map(item => (
                <button
                  key={item}
                  onClick={() => {
                    document.getElementById(item === 'Home' ? 'Home' : item.toLowerCase())?.scrollIntoView({ behavior: "smooth" });
                    setMobileMenu(false);
                  }}
          ...
          <section id="Home" className="relative w-full h-screen min-h-[640px] overflow-hidden bg-black">
            <div className="absolute inset-0 bg-gradient-to-br from-[#050505] via-[#111111] to-[#0a0a0a]">
              <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:24px_24px]" />
              <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#333333] rounded-full mix-blend-screen filter blur-[120px] opacity-30" />
              <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-[#222222] rounded-full mix-blend-screen filter blur-[100px] opacity-40" />
            </div>
            ...
            <div className="relative z-10 w-full h-full flex flex-col justify-between px-6 md:px-12 pt-24 pb-10">
              <h1
                ...
                className={`font-display uppercase leading-[0.85] tracking-[-0.03em] text-[18vw] md:text-[12vw] lg:text-[10rem] cursor-pointer transition-all duration-300 ${colors[colorMode]}`}
              >
                {displayed || "\u00A0"}
              </h1>

              <p className="md:absolute md:top-28 md:right-12 mt-4 md:mt-0 text-right text-3xl md:text-4xl lg:text-5xl leading-[1.05] max-w-md font-[Poppins] font-bold tracking-wide text-transparent bg-clip-text bg-[length:200%_auto] bg-gradient-to-r from-white via-white/60 to-white animate-shine">
                Creating
                <br />
                Websites
                <br />
              ...
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mt-auto">
                <p className="relative text-sm sm:text-base lg:text-xl leading-relaxed max-w-md font-[Poppins] font-medium tracking-wide text-transparent bg-clip-text bg-[length:200%_auto] bg-gradient-to-r from-white via-white/60 to-white animate-shine">
